...
int runGame(int levelNumber)
{
	...
	std::array<std::array<char, 30>, 22>encounterMap = testEncounter.getLevel(levelNumber);
	...
	int levelCheckeVar = (encounterMap[21][29] - 48);
	//std::cout << levelCheckeVar;
	while (levelCheckeVar == levelNumber)//currently loops through and gets next move and prints the map will go until they find the astrics can be changed later
	{
		levelCheckeVar = (encounterMap[21][29] - 48);
